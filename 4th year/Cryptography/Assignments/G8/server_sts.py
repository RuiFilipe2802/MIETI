...
class Server(object):
    ...
    async def process(self, reader, writer):
        try:
            ...
            # verify signature with public rsa
            cs_public_key = client_public_key_bytes + server_public_key_bytes 
            rsa1.sign_verify("rsa_server.txt", signed_pt, cs_public_key)

            # sends ct so client verifies signature too
            sc_public_key = server_public_key_bytes + client_public_key_bytes
            signature2 = rsa1.sign_message("rsa_server.txt", sc_public_key)
        
            signed_ct2 = cipher.encrypt(signature2)
